...
public class PersonDataGUI {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getClickCount() > 1) {
				JOptionPane.showMessageDialog(f,
						people[table.getSelectedRow()] + ".\nThe age is " + table.getValueAt(row, 2).toString()
								+ " years old" + ".\nThe social security number is "
								+ table.getValueAt(row, 1).toString() + ".\nThe height is "
								+ table.getValueAt(row, 3).toString() + " centimeters tall" + ".\nThe weight is "
								+ table.getValueAt(row, 4).toString() + " kilograms");
	...
	private void resetTable() {
		...
		this.personDataModel = new PersonDataModel(defaultPeople);
